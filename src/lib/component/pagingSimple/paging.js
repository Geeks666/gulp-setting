define(['jquery'], function ($) {
  ...
  Paging.prototype = {
    ...
    format: function () {
      var html = '<ul>'
      this.settings.firstTpl && (html += '<li class="js-page-first js-page-action ui-pager" >' + this.settings.firstTpl + '</li>');
      html += '<li class="js-page-prev js-page-action ui-pager">' + this.settings.prevTpl + '</li>';
      if (this.settings.pageCount === 'auto') {
        if (this.pagecount > 6) {
        ...
        if (this.settings.pageCount !== 0) {
          if (this.pagecount > this.settings.pageCount) {
            var tempStart = 1 + ((Math.ceil(this.current / this.settings.pageCount) - 1) * this.settings.pageCount),
              tempEnd = this.pagecount > (tempStart + this.settings.pageCount - 1)
                ? (tempStart + this.settings.pageCount - 1)
                : this.pagecount;
      ...
      this.settings.lastTpl && (html += '<li class="js-page-last js-page-action ui-pager">' + this.settings.lastTpl + '</li>');
      html += '</ul>';
