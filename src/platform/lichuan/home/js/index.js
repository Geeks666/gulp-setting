  ...
  define('', ['jquery', 'template', 'service', 'tool', 'banner', 'textScroll', 'scrollNav', 'center', 'main', 'footer', 'header', 'layer', 'indexApp'],
    function ($, template, service, tools, banner, textScroll, scrollNav, center, main, footer, header, layer, indexApp) {
      ...
      $(function () {
        ...
        //热门资源
        getResource($("#resource_list"), 'resourceList', "140");
        //教育新闻
       ...
       * @param phaseId
       */
      function getResource($obj, temId, phaseId) {
        $.ajax({
          url: service.htmlHost + '/pf/api/qtapp/res_resource?phaseId=' + phaseId + '&count=3',
          type: 'GET',
          success: function (data) {
            if (data && data.code == "success") {
              var html = '';
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data && data.code == "success") {
              ...
              if ($(".slide-pic").find("li img").length > 0) {
                var loadPicTimer = setInterval(function () {
                  if ($(".slide-pic").find("li img").eq(0).width() > 0) {
                    clearInterval(loadPicTimer);
                    $.each($(".slide-pic").find("li img"), function (index) {
