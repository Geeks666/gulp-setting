...
require(['customConf'], function (configpaths) {
  ...
  define('', ['jquery', 'paging', 'service', 'common','dataResource/moduleDetailData.js'],
    function ($, paging, service, common,moduleDetailData) {
      ...
      function moduleTable(flag, time, name, scope, pageNo, pageSize, isNext) {
        ...
        var url = module_detail[flag].url;
       /* $.ajax({
          type: "GET",
          dataType: "json",
          url: url,
          success: function (json) {*/
            ...
            if (moduleDetailData[flag]['code'] == 200) {
              if (moduleDetailData[flag].data.dataList.length > 0) {
                $.each(moduleDetailData[flag].data.dataList, function (i, n) {
                  if (i % 2 == 0) {
                    trColor = "odd";
                  }
                  else {
                  ...
                  if (flag == 'space') {
                    if ((pageNo == moduleDetailData[flag].data.pageCount) && (i == moduleDetailData[flag].data.dataList.length - 1)) return;
                  }
                ...
                if (flag == 'space' && _tabVal == 1 && pageNo == moduleDetailData[flag].data.pageCount) {
                  $('.tableTotal0')
                    .html('<div class="total">合计</div>\
                <div class="total1">' + moduleDetailData[flag].data.dataList[moduleDetailData[flag].data.dataList.length - 1].spaceTotal + '</div>\
              <div class="total2">' + moduleDetailData[flag].data.dataList[moduleDetailData[flag].data.dataList.length - 1].openPersonSpaceTotal + '</div>\
              <div class="total2">' + moduleDetailData[flag].data.dataList[moduleDetailData[flag].data.dataList.length - 1].usePersonSpaceTotal + '</div>\
              <div class="total2">' + moduleDetailData[flag].data.dataList[moduleDetailData[flag].data.dataList.length - 1].visitPersonSpaceTotal + '</div>\
              <div class="total2">' + moduleDetailData[flag].data.dataList[moduleDetailData[flag].data.dataList.length - 1].openGroupSpaceTotal + '</div>\
              <div class="total2" style="border-right:none;">' + moduleDetailData[flag].data.dataList[moduleDetailData[flag].data.dataList.length - 1].visitGroupSpaceTotal + '</div>')
                    .show();
                } else {
                  $('.tableTotal0').hide();
                }
                // 用户分页
                if (moduleDetailData[flag].data.pageCount > 1 && pageIsShow) {
                  ...
                  pageToolUserCity.init({
                    ...
                    count: moduleDetailData[flag].data.totalSize, callback: function (current) {
                      moduleTable('user', $('#date').val(), '', role, current, '10');
                    }
                  });
                } else if (moduleDetailData[flag].data.pageCount == 1) {
                  $('#pageToolUser' + scope).html('');
                }
                // 空间分页
                if (moduleDetailData[flag].data.pageCount > 1 && pageIsShow) {
