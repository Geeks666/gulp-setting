...
require(['customConf'], function (configpaths) {
  ...
  define('',['jquery', 'service', 'tools', 'common','dataResource/spaceEchartData.js'],
    function ($, service, tools, common,spaceEchartData) {
      ...
      function spaceUseData(spacetype, type, spaceId) {
        ...
        /*$.getJSON(service.prefix + '/space' + '/case/' + spacetype + '?' + spaceType + 'errorDomId=' + spaceId)
          .success(function (result) {*/
            if (spaceEchartData.spaceCaseData[spacetype]['code'] == 200 && spaceEchartData.spaceCaseData[spacetype].data.length != 0) {
              render(spaceEchartData.spaceCaseData[spacetype].data, spaceId);
            }
      ...
      function spaceUseTrendData(type, startDate, endDate, trendId) {
        ...
        /*$.getJSON(service.prefix + '/space/trend' + '?type=' + type + '&startDate=' + startDate + '&endDate=' + endDate +
          ...
          .success(function (result) {
            ...
            if (spaceEchartData.spaceTrendData[type]['code'] == 200) {
              render(spaceEchartData.spaceTrendData[type].data, trendId);
            }
         /* });*/
      }
      ...
      function getSpaceQuantity(detail) {
        ...
        $.ajax({
          ...
          success: function (json) {*/
            if (spaceEchartData.spaceCountData['code'] == 200) {
              // 个人空间
              $('.personal_space').html(spaceEchartData.spaceCountData.data.person.spaceTotal);
              $('.student_space').html(spaceEchartData.spaceCountData.data.person.studentSpaceNum + '个');
              $('.parent_space').html(spaceEchartData.spaceCountData.data.person.parentSpaceNum + '个');
              $('.teacher_space').html(spaceEchartData.spaceCountData.data.person.teacherSpaceNum + '个');
              $('.teaching_space').html(spaceEchartData.spaceCountData.data.person.researchSpaceNum + '个');
              // 群组空间
              $('.group_space').html(spaceEchartData.spaceCountData.data.group.spaceTotal);
              $('.education_space').html(spaceEchartData.spaceCountData.data.group.areaSpaceNum + '个');
              $('.school_space').html(spaceEchartData.spaceCountData.data.group.schoolSpaceNum + '个');
              $('.class_space').html(spaceEchartData.spaceCountData.data.group.classSpaceNum + '个');
              $('.subject_space').html(spaceEchartData.spaceCountData.data.group.subjectSpaceNum + '个');
            }
         /* },
          error: function (json) {
